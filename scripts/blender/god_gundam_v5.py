#!/usr/bin/env python3
"""
God Gundam - Version 5
Improved wing angles, more detail, refined proportions
Run: blender -b --python god_gundam_v5.py --python-exit-code 1 -- output=/tmp
"""
import bpy
import sys
import json
...
def wing_blade(name, loc, dims, m, rot_x=0, rot_y=0, rot_z=0):
    """Create an angled wing blade."""
    bpy.ops.mesh.primitive_cube_add(size=1, location=loc)
    o = bpy.context.active_object
    o.name = name
    o.dimensions = dims
...
def build(mats):
    ...
    # =================== WING BINDERS - SPREAD OUT DRAMATICALLY ===================
    # The wings should spread outward and backward like in the reference images
    # 4 wings: 2 upper (angled up and out), 2 lower (angled down and out)

    wing_data = [
        # (name, side_mult, is_upper, y_rot, x_rot)
        ('WLU', -1, True, -45, -20),   # Left Upper - angled left and back
        ('WRU', 1, True, 45, -20),     # Right Upper - angled right and back
        ('WLL', -1, False, -35, 10),   # Left Lower - angled left, slightly down
        ('WRL', 1, False, 35, 10),     # Right Lower - angled right, slightly down
    ]

    for name, sx, is_upper, y_rot, x_rot in wing_data:
        base_x = sx * 0.2
        base_y = 0.24
        base_z = back_z + (0.14 if is_upper else -0.1)

        # Wing mount
        parts.append(box(f"{name}Mount", (base_x + sx * 0.06, base_y + 0.08, base_z), (0.12, 0.18, 0.14), mats['dark_gray']))

        # Main wing blade - LONG, angled outward
        # The blade extends in the direction of the rotation
        blade_length = 1.0
        blade_x = base_x + sx * 0.5
        blade_y = base_y + 0.55
        blade_z_adj = base_z + (0.08 if is_upper else -0.04)

        parts.append(wing_blade(f"{name}Blade", (blade_x, blade_y, blade_z_adj),
                                (0.1, blade_length, 0.24), mats['white'],
                                x_rot, y_rot, 0))

        # Red trim on wing - positioned along the blade
        red_x = blade_x + sx * 0.04
        red_y = blade_y - 0.1
        parts.append(wing_blade(f"{name}Red", (red_x, red_y, blade_z_adj),
                                (0.07, blade_length * 0.45, 0.14), mats['red'],
                                x_rot, y_rot, 0))

        # Wing tip accent
        tip_x = blade_x + sx * 0.02
        tip_y = blade_y + blade_length * 0.4
        parts.append(wing_blade(f"{name}Tip", (tip_x, tip_y, blade_z_adj),
                                (0.06, 0.18, 0.12), mats['light_gray'],
                                x_rot, y_rot, 0))
