...
function validateManifestExists(): { passed: boolean; message?: string; manifest?: AssetManifest } {
  ...
  try {
    ...
    return {
      passed: false,
      message: `Invalid asset manifest: ${error instanceof Error ? error.message : String(error)}`,
    }
  }
}
...
function validateDirectoryStructure(): ValidationResult {
  ...
  const requiredDirs = [CONFIG.chunksDir, CONFIG.propsDir, path.dirname(CONFIG.manifestPath)]
