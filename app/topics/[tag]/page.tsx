...
export async function generateMetadata({ params }: PageProps) {
  ...
  const getLatestDate = (): string | undefined => {
    ...
    for (const book of books) {
      const bookDate =
        book.dateRead || book.dateStarted || (book.year ? `${book.year}-01-01` : undefined)
...
function TopicStat({ label, value }: { label: string; value: number }) {
  return (
    <div className="rounded-xl border border-border-1 bg-surface-1 p-4">
      <div className="text-xs uppercase tracking-[0.2em] text-text-3">{label}</div>
      <div className="mt-2 font-satoshi text-2xl font-semibold text-text-1">{value}</div>
    </div>
  )
}
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="font-satoshi text-lg font-medium text-text-1">{book.title}</h3>
      ...
      {book.whyILoveIt && (
        <p className="mt-3 text-sm text-text-2 line-clamp-3">{book.whyILoveIt}</p>
      )}
    </Link>
  )
}
