...
const InteractiveWorld = dynamic(
  () => import('@/components/interactive/InteractiveWorld').then((m) => m.InteractiveWorld),
  {
    ssr: false,
    ...
    <div className="flex h-screen w-screen flex-col items-center justify-center bg-bg-0 p-6">
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="text-center">
          <h1 className="font-satoshi text-4xl font-medium text-text-1">Enter the World</h1>
        ...
        <div className="rounded-lg border border-surface-1 bg-surface-1/30 p-4 font-mono text-xs text-text-3">
          ...
          <div className="flex justify-between">
            ...
            <span className="capitalize text-warm">{capabilities.suggestedTier}</span>
          </div>
        ...
        <div className="space-y-3">
          <p id="quality-tier-label" className="block text-sm font-medium text-text-2">
            Quality Tier
          </p>
          <div
            className="grid grid-cols-4 gap-2"
            role="group"
            aria-labelledby="quality-tier-label"
      ...
      {/* Return link */}
      <div className="mt-12">
        <Link href="/" prefetch={false} className="text-sm text-text-3 hover:text-text-2">
  ...
  return (
    <div className="flex h-screen w-screen flex-col items-center justify-center bg-bg-0 p-6">
      <div className="max-w-md space-y-6 text-center">
        <h1 className="font-satoshi text-3xl font-medium text-text-1">WebGL Not Available</h1>
        <p className="text-text-2">
          The Interactive experience requires WebGL2, which isn&apos;t available on this device or
          browser.
        </p>
        <div className="space-y-3">
          <Link
          ...
          <p className="text-xs text-text-3">
            The Normal site has all the same content in a fast, accessible format.
          </p>
        </div>
      </div>
    </div>
...
function ErrorUI({ message, onRetry }: { message: string | null; onRetry: () => void }) {
  return (
    <div className="flex h-screen w-screen flex-col items-center justify-center bg-bg-0 p-6">
      <div className="max-w-md space-y-6 text-center">
        <h1 className="font-satoshi text-3xl font-medium text-text-1">Something Went Wrong</h1>
        <p className="text-text-2">{message ?? 'An error occurred while loading the 3D world.'}</p>
        <div className="flex flex-col gap-3">
          <button
            onClick={onRetry}
