  ...
  return (
    <group position={[0, 0.01, 0]}>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} receiveShadow>
        ...
        <meshStandardMaterial color="#3d1a1a" roughness={0.9} metalness={0} />
      </mesh>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.003, 0]}>
        ...
        <meshStandardMaterial color="#5c3d1e" roughness={0.85} metalness={0.1} />
      </mesh>
    </group>
  )
}
...
function MoonlitWindow() {
  return (
    <group position={[ROOM_WIDTH / 2 - 0.15, 4, 4]}>
      ...
      <pointLight position={[0.5, 0, 0]} intensity={0.15} color="#8899bb" distance={6} decay={2} />
    </group>
  )
}
...
function Floor() {
  return (
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0, 0]} receiveShadow name="ground">
      ...
      <meshStandardMaterial color="#2d2418" roughness={0.7} metalness={0.1} />
    </mesh>
  )
}
...
function BookshelfCollider({
  ...
  return (
    <RigidBody type="fixed" position={position} rotation={[0, rotation, 0]}>
      <CuboidCollider
        args={[SHELF_WIDTH / 2, SHELF_HEIGHT / 2, SHELF_DEPTH / 2]}
...
function Book({
  ...
  useFrame(() => {
    ...
    meshRef.current.position.z =
      instance.position[2] + currentOffset + (targetOffset - currentOffset) * 0.1
  })
...
export function LibraryRoom({ debug = false, onDoorActivate, onContentSelect }: LibraryRoomProps) {
  ...
  return (
    <group name="room-library">
      ...
      {/* Books */}
      {!loading && books.length > 0 && <Books books={books} onBookSelect={handleBookSelect} />}
