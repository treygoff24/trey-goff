 ...
 */
function SquatRack({ weight, position }: { weight: number; position: [number, number, number] }) {
  return (
 ...
 */
function PlaqueWall({ lifts, total }: { lifts: LiftsManifestEntry[]; total: LiftRecord }) {
  ...
  return (
    <group position={[0, 2.5, -ROOM_DEPTH / 2 + 0.5]}>
      ...
      {squat && <PRPlaque lift={squat.lift} pr={squat.pr} position={[-2.5, 0, 0]} />}
      {bench && <PRPlaque lift={bench.lift} pr={bench.pr} position={[0, 0, 0]} />}
      {deadlift && <PRPlaque lift={deadlift.lift} pr={deadlift.pr} position={[2.5, 0, 0]} />}
...
export function GymRoom({ debug = false, onDoorActivate }: GymRoomProps) {
  ...
  return (
    <group name="room-gym">
      ...
      {/* PR Wall */}
      {!loading && liftsData && <PlaqueWall lifts={liftsData.lifts} total={liftsData.total} />}
