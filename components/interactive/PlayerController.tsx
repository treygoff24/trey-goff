...
function useDesktopControls(
  ...
  useEffect(() => {
    ...
    const handleClick = () => {
      if (!isLocked.current) {
        void Promise.resolve(gl.domElement.requestPointerLock()).catch((error: unknown) => {
          console.error('[PlayerController] Failed to request pointer lock:', error)
        })
      }
    }
...
export function PlayerController({
  ...
  useFrame((state, delta) => {
    ...
    if (!isMobile) {
      ...
      if (target) {
        diffVec.current.set(
          target.x - currentPosVec.current.x,
          0,
          target.z - currentPosVec.current.z,
        )
