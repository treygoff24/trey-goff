...
function OverlayHeader({
  ...
  return (
    <div className="flex items-start justify-between border-b border-border-1 pb-4">
      <div>
        <p className="text-xs font-medium uppercase tracking-wider text-warm">{typeLabel}</p>
...
function OverlayTags({ tags }: { tags: string[] }) {
  return (
    <div className="flex flex-wrap gap-2">
      {tags.map((tag) => (
        <span key={tag} className="rounded-full bg-surface-1 px-3 py-1 text-xs text-text-2">
          {tag}
        </span>
      ))}
    </div>
    ...
    >
      ...
      >
        ...
        <div className="mt-4 space-y-4">
          {content.image && <OverlayImage src={content.image} alt={content.title} />}

          {content.description && <p className="text-text-2">{content.description}</p>}

          {content.meta && Object.keys(content.meta).length > 0 && (
            <OverlayMeta meta={content.meta} />
          )}

          {content.tags && content.tags.length > 0 && <OverlayTags tags={content.tags} />}
