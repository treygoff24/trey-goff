...
function CameraController() {
  ...
  useFrame((_, delta) => {
    ...
    sceneState.warp.factor = THREE.MathUtils.lerp(sceneState.warp.factor, targetFactor, delta * 2.5)
    ...
    if (sceneState.warp.factor > CONFIG.camera.shakeThreshold) {
      ...
      shakeOffset.current.x =
        Math.sin(shakeTime.current * 1.1) * Math.cos(shakeTime.current * 0.7) * intensity
      shakeOffset.current.y =
        Math.cos(shakeTime.current * 0.9) * Math.sin(shakeTime.current * 1.3) * intensity
    } else {
...
function StarField() {
  ...
  useFrame((_, delta) => {
    ...
    const { baseSpeed, warpMultiplier, respawnThreshold, respawnDepth, fieldSize, count } =
      CONFIG.stars
    ...
    for (let i = 0; i < count; i++) {
      ...
      dummy.current.position.set(positions.current[ix]!, positions.current[ix + 1]!, z)
...
function FloatingPlanet({
  ...
  useFrame((_, delta) => {
    ...
    const { warpMultiplier, moveSpeed, respawnThreshold, respawnDepth, edgeZone, verticalZone } =
      CONFIG.planets
    const warpFactor = 1 + sceneState.warp.factor * warpMultiplier
  ...
  useFrame((state) => {
    ...
    materialsRef.current.forEach((mat, i) => {
      if (mat) {
        ...
        mat.uniforms.uOffset!.value.set(data.offset[0] + parallaxX, data.offset[1] + parallaxY)
      }
    })
  ...
  return (
    <group ref={groupRef}>
      {nebulaData.map((data, i) => (
        <mesh key={i} position={data.position} rotation={[0, 0, Math.PI * 0.25 * i]}>
...
function SpaceScene() {
  return (
    <>
      ...
      <fog
        attach="fog"
        args={[COLORS.background, CONFIG.postProcessing.fog.near, CONFIG.postProcessing.fog.far]}
      />
    </>
  )
}
    ...
    <div className="fixed inset-0 -z-10 h-full w-full bg-bg-0">
      ...
      <div
        ...
        style={{
          ...
          background:
            'radial-gradient(circle at center, transparent 0%, rgba(255,184,107,0.1) 100%)',
        }}
      />
    </div>
  )
