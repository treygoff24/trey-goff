...
describe('A11y Focus-Visible Styles', () => {
  ...
  describe('NoteCard anchor link', () => {
    it('should have focus-visible styles for keyboard users', () => {
      ...
      const hasFocusVisible =
        /href=\{`#\$\{slug\}`\}[\s\S]{0,200}className="[^"]*focus-visible:(opacity-100|text-)/.test(
          content,
        )
  ...
  describe('main element tabIndex', () => {
    it('should not have tabIndex={-1} outline flash issue', () => {
      ...
      if (mainTag.includes('tabIndex={-1}')) {
        ...
        assert.ok(
          hasOutlineNone,
          'main with tabIndex={-1} should have outline-none to prevent flash',
        )
      } else {
        assert.ok(
          !mainTag.includes('tabIndex'),
          'main element should not have tabIndex to avoid focus outline flash',
        )
      }
    })
  })
