...
describe('ProjectCard accessibility', () => {
  ...
  it('should use description list for label-value pairs', () => {
    assert.match(source, /<dl[>\s]/, 'ProjectCard should contain a description list element')
  })

  it('should use dt elements for labels', () => {
    assert.match(source, /<dt[>\s]/, 'ProjectCard should contain dt elements for labels')
  })

  it('should use dd elements for values', () => {
    assert.match(source, /<dd[>\s]/, 'ProjectCard should contain dd elements for values')
  })
  ...
  it('should not use p tags for label-value display in Problem/Approach sections', () => {
    ...
    if (problemApproachSection) {
      ...
      const hasLabelPTag = /<p className="text-xs uppercase/.test(sectionContent)
      assert.strictEqual(
        hasLabelPTag,
        false,
