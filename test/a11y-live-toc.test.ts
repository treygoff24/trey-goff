...
describe('A14: CommandPalette loading aria-live', () => {
  it('loading indicator has aria-live="polite"', () => {
    ...
    const hasAriaLive = /isLoading[\s\S]*?<div[^>]*aria-live="polite"[\s\S]*?Loading search/i.test(
      content,
    )
...
describe('A15: Mobile TOC toggle aria-expanded', () => {
  it('toggle button has aria-expanded attribute', () => {
    ...
    const hasAriaExpanded = /MobileTableOfContents[\s\S]*?<button[\s\S]*?aria-expanded=/i.test(
      content,
    )

    assert.ok(hasAriaExpanded, 'Mobile TOC toggle button must have aria-expanded attribute')
  })
})
...
describe('A18: ContentOverlay focus restoration', () => {
  it('saves focus reference on open', () => {
    ...
    const hasFocusSave =
      /const\s+\w*[Ff]ocus\w*Ref\w*\s*=\s*useRef/.test(content) ||
      /previouslyFocusedElement\s*=\s*document\.activeElement/.test(content) ||
      /\w+Ref\.current\s*=\s*document\.activeElement/.test(content)

    assert.ok(hasFocusSave, 'ContentOverlay must save the previously focused element when opening')
  })
  ...
  it('restores focus on close', () => {
    ...
    const hasFocusRestore =
      /\w+Ref\.current\.focus\(\)/.test(content) ||
